...
int TCPServer::Receive(SOCKET client, char *&data, SSL *c_SSL) {
  ...
  if (c_SSL) {
    ...
    while (TotalReceived < std::stoi(SizePacket)) {
      int ResultData =
          SSL_read(c_SSL, data + TotalReceived, std::stoi(SizePacket));
      if (ResultData <= 0) {
    ...
    while (TotalReceived < std::stoi(SizePacket)) {
      int ResultData =
          recv(client, data + TotalReceived, std::stoi(SizePacket), 0);
      if (ResultData == -1) {
        std::cerr << "Failed to receive data from client" << std::endl;
        return -1;
      }
...
static bool Encrypt(SSL_CTX *ctx) {
  ...
  X509_NAME_add_entry_by_txt(name, "C", MBSTRING_ASC, (unsigned char *)"NL", -1,
                             -1, 0);
  X509_NAME_add_entry_by_txt(name, "O", MBSTRING_ASC,
                             (unsigned char *)"CPP_Networking", -1, -1, 0);
  X509_NAME_add_entry_by_txt(
      name, "CN", MBSTRING_ASC,
      (unsigned char *)"www.github.com/timvnaarden/CPP_NETWORK_WRAPPER", -1, -1,
      0);
